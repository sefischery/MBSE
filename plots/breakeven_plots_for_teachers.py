...
# Power generation when no limitations on batteries and wind power - divided by 250
...
# Data
plt.plot(percentage, alt2_total_generation1, label='1 wind turbine', linewidth=2)
# plt.plot(percentage, alt_total_generation2,label='2 wind turbines', color="green")
plt.plot(percentage, alt2_total_generation6, label='6 wind turbines', linewidth=2)
plt.hlines(power_consumed, 0, 100, label='Power consumed', color='0.4', linestyles=':')
# Labels
ax.set_xlabel('Amount of solar cells [%]')
ax.set_ylabel('Power [MW]')
plt.yticks(plt.yticks()[0], [int(i / 1000000) for i in plt.yticks()[0]])
plt.xlim([0, 100])
plt.ylim(ymin=0)
# Plot layout
...
#plt.show()
fig.savefig('output/solar-cell-break-even.pdf')
