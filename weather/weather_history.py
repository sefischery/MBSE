...
class WeatherLog:
    ...
    def get_solar_rad(self, time):
        solar_before = self.dataFrame.loc[self.__datetime_to_key_string(time), 'solar_rad']
        solar_after  = self.dataFrame.loc[self.__datetime_to_key_string(time + datetime.timedelta(hours=1)), 'solar_rad']
    ...
    def get_wind_speed(self, time):
        wind_before = self.dataFrame.loc[self.__datetime_to_key_string(time), 'wind_spd']
        wind_after  = self.dataFrame.loc[self.__datetime_to_key_string(time + datetime.timedelta(hours=1)), 'wind_spd']
